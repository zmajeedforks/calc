// calc2_parser.minimal.gtest.cpp
...
namespace {
using namespace calc2min;
using tk = Calc2MinParser::token;
using Kind = Calc2MinParser::token_kind_type;
using Sym = Calc2MinParser::symbol_type;
...
struct Calc2Min_BisonNoFlex: Test {
  ...
  static int parse(span<const Tok> toks) {
    ...
    size_t i = 0;
    Calc2MinParser parser([&toks, &i](LexParam& param) -> Sym {
      auto& loc = param.loc;
      loc.step();
      loc.columns();
      ...
      return Calc2MinParser::make_YYEOF(loc);
    },
    lexParam);
